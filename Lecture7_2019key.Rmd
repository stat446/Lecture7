...
The point estimates for $t_y$ and $\bar{y}_u$ are `r round(t_y)` and `r round(ybar_r, 1)`.
\vfill
\newpage
...
The standard errors for the estimates of $t_y$ and $\bar{y}_u$ are `r round(se_t)` and `r round(se_ybar, 2)`.
\vfill
...
If the researcher wants to estimate $t_y$ or $\bar{y}_U$, the main sampling question is *`When is it worth the additional effort and expense to collect information about $X$ instead of just using a SRS estimator $\hat{\bar{y}}_U$ or $\hat{t}$ which does not require knowledge about X?*
\vfill
...
It can be shown that if $R > \frac{CV(\bar{x})}{CV(\bar{y})}$, _then the variance of the ratio estimator is smaller than the variance of the SRS estimator._
\vfill
...
1. You must be able to simultaneously observe $X$ and $Y$ values that are `roughly proportional' to each other. _That is, there is a strong positive linear relationship between $Y$ and $X$ that passes through the origin (zero intercept)._
\vfill
...
If there is a linear relationship between $Y$ and $X$ and the intercept is not zero or the correlation between $X$ and $Y$ is negative, _then a regression estimator should be considered._
\vfill
...
It is common to want estimates of a mean or total for subpopulations. _The subpopulations are called domains._
\vfill
...
Let $S_d$ be the set of _sample units_ in domain $d$ and let $n_d$ be the number of sample units in domain $d$. Natural estimators for the domain mean $\bar{y}_{Ud}$ and the domain total $t_{yd}$ are
	\begin{equation}
	*\hat{\bar{y}}_{Ud} = \left( \sum_{i \in S_d} y_i \right) / n_d = \bar{y}_d* \hspace{2.5cm} *\hat{t}_{yd} = \frac{N_d}{n_d} \left(\sum_{i \in S_d} y_i \right) = N_d \bar{y}_d*
	\end{equation}
\vfill
...
For the SRS settings earlier in the course, the sample size $n$ was fixed. _For a domain, the sample size $n_d$ is not fixed, but is a random variable. That is, if we took different random samples of size $n$, we would get different values for $n_d$._
\vfill

_Because $n_d$ is a random variable, we cannot use the SRS variance formulas from Section 2. To find the variance, $V(\bar{y}_D)$, we need to see that $\bar{y}_D$ is a ratio estimator._
...
Assume a SRS of n pairs $(x_1, y_1), \dots, (x_n, y_n)$ is selected from a population of $N$ pairs of $(x,y)$ data. The goal of regression estimation is to _take advantage of a linear relationship between $x$ and $y$ to improve estimation of the $t_y$ or $\bar{y}_U$._
\vfill
...
Unlike ratio estimation, there is no assumption of a zero intercept in the linear relationship of a positive slope. We assume a linear form for the relationship between $y$ and $x$:
	...
	*y_i = B_0 + B_1 x_i + \epsilon_i,*
	\end{equation}
	*for intercept $B_0$, slope $B_1$, and $\epsilon_i$ is the deviation between $y_i$ and $B_0 + B_1x_i$.*
\vfill

We assume that 

1. the mean of the $\epsilon_i's$ _is zero and_ 
\vfill

2. the $\epsilon_i's$ are uncorrelated with the $x_i's$. _This implies that there is not systematic relationship beween the $\epsilon_i's$ and the $x_i's$._ 
\vfill
...
To estimate $\bar{y}_U$, we first must get estimates $\hat{B}_0$ and $\hat{B}_1$ of the true intercept $B_0$ and slope $B_1$. We use the least squares estimates: 
	\begin{equation}
		*\hat{B}_0 = \bar{y} - \hat{B}_1 \bar{x}* \hspace{3cm} \hat{B}_1 = \frac{\sum_{i=1}^N (x_i - \bar{x})(y_i - \bar{y})}{\sum_{i=1}^n (x_i - \bar{x})^2} 
	\end{equation}
\vfill
...
When $\bar{x}_U$ is known, our estimate $\hat{\bar{y}}_{reg}$ of the population mean is:
	\begin{equation}
	*\hat{\bar{y}}_{reg} = \hat{B}_0 + \hat{B}_1 \bar{x}_U*
	\end{equation}
	When $\bar{x}_U$ is \emph{unknown}, replace $\bar{x}_U$ with $\bar{x}$, then *$\hat{\bar{y}}_{reg} = \bar{y}$.*
\vfill
...
Case 2: A $k^{th}$ order polynomial is formed for one regression variable $x$ with the model
	\begin{equation}
	*y = B_0 + B_1 x + B_2 x^2 + \dots B_k x^k + \epsilon*
	\end{equation}
...
2. To estimate $\bar{y}_U$, we make predictions using the mean of each of our covariates
	\begin{equation}
	*\hat{\bar{y}}_{reg} = \hat{B}_0 + \hat{B}_1 \bar{x}_{1U}, \dots, \hat{B}_k \bar{x}_{kU}.*
	\end{equation}
\vfill
...
1. Again find the least-squares estimates of $(\hat{B}_0, \hat{B}_1, \dots, \hat{B}_k)$ to compute the prediction model:
	\begin{equation}
	*\hat{y} = \hat{B}_0 + \hat{B}_1 x + \dots \hat{B}_k x^k*
	\end{equation}
\vfill

2. To estimate the mean value, $\bar{y}_U$, use the mean for $x$
	\begin{equation}
	*\hat{\bar{y}}_{reg} = \hat{B}_0 + \hat{B}_1 \bar{x}_U + \dots \hat{B}_k \bar{x}_U^k*
	\end{equation}
\vfill
...
```
Thus the mean weight of alligators, given the mean length is 90 inches is `r round(y_pred,1)` pounds.
\vfill
...
Thus a 95% confidence interval for the mean alligator length is `r round(ci, 2)`.
...
- If $x$ and $y$ are strongly correlated, most of the variability in the $y$ values *can be explained by the regression line and there will be very little random variability about the regression line. That is MS(regression) is large relative to MSE. (The same idea holds in a multiple regression setting).*
\vfill
